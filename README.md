...
当前公开下载版本：**1.0.2**；仓库源码构建版本：**1.0.3**。支持 Windows 10 / 11（64 位）。普通用户直接下载安装包，无需配置 Node.js 或 mpv。
...
- **播放引擎**：mpv 内核，覆盖 MP4/MKV/AVI/MOV/TS/WebM 等主流容器；硬件解码自动协商（D3D11VA / DXVA2 / NVDEC），支持软件解码回退和播放失败重试。
- **HDR 画质决策链**：HDR10 / HLG 自动直通或色调映射，7 种 Tone Mapping 算法可选，支持目标峰值 / 对比度恢复 / 饱和度等实时调参。
- **DLNA 投屏**：提供 MediaRenderer（DMR）及状态同步、控制权仲裁、后台待机接收；换集保留全屏、音量和静音。当前仅允许受控的局域网直接媒体文件，不支持 HLS/DASH 清单，详见版本说明。
- **媒体库**：后台扫描与纯本地刮削海报墙（文件名 / nfo / 同目录封面解析，无在线依赖），剧集识别、无封面自动抽帧。
- **播放画面优先**：浏览页保留明暗主题与氛围效果，播放页停用装饰动画；自绘控制层支持进度预览及即时反馈的音量拖动。
- **缩略图预览**：进度条悬停显示真实帧缩略图 + 章节标记 + 时间气泡。
- **NAS / SMB 在线浏览**：UNC 路径直连点播，无需入库。
- **工程化细节**：mpv 独立子进程（异常退出最多重试 2 次）、防火墙自检、系统托盘待机、续播记忆。
...
│   └── package.json
├── docs/                     # 设计与规格文档（5 份）
├── build-tools/              # 固定版本的发布工具与独立锁文件
├── runtime/                  # mpv 运行时 + 测试素材（gitignore，需自行下载）
│   ├── mpv/mpv.exe
│   └── library-test/
└── .github/workflows/        # Windows 自动检查
```
...
> 启用 DLNA 时会尝试创建仅限专用网络、本地子网和当前程序的入站规则。权限不足时显示防火墙提示，需要在 Windows 防火墙中允许应用访问专用网络。
...
| `npm run build` 报 TypeScript 错误 | 渲染层类型检查未通过 | 运行 `npm run typecheck` 查看具体错误 |
...
当前提供 **Aqua** 内置主题、明暗外观与用户自定义预设。视觉控制台可调整流体、环境光、粒子和播放态效果；历史六主题方案不再代表当前注册表。
...
投屏地址默认仅允许局域网 HTTP(S)。媒体通过本地受控入口转发，DNS 与重定向逐跳校验；事件回调必须指向订阅控制端。当前投屏支持直接媒体文件，拒绝 HLS/DASH 和外部引用清单。
...
1. 设置页 → `媒体库` 分组 → `添加文件夹`。后台扫描显示进度；离线或不完整目录保留旧记录，源目录移出配置后才移除相应条目。
...
发布工具使用独立的 build-tools 工作区与锁文件。开发和打包共同使用根目录 runtime/mpv；打包前校验固定散列。完整步骤见 [发布指南](docs/RELEASE.md)。

`npm run check` 包含回归测试、HDR/视觉测试、类型检查和构建。安装包还需验证安装、升级、文件关联和真机输出后再公开发布。
...
- 当前构建版本：**1.0.1**，汇总审查修复、播放观感优化、投屏换集状态保留和音量条改进。参见 [v1.0.1 版本说明](docs/releases/RELEASE-NOTES-v1.0.1.md)；是否公开发布以发行页为准。
- 本地安装包位于 `release/v1.0.1/`，历史产物在 `release/archive/`，构建中间文件在 `.release-build/`。
- 2026-09-03 审查发现的 F01–F16 已进行代码修复，逐项验证与限制见 [修复记录](docs/09-逐项修复与验证记录.md)。
- 类型检查成为 build 的前置门槛；新增数据库、播放会话、网络和扫描回归检查。
- HDR/GPU、手机/NAS、安装升级和卸载仍需要与候选版本对应的真机验收。历史验收记录不代表新代码已经完成真机复验。
